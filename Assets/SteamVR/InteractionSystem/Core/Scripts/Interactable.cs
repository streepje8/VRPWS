...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Interactable : MonoBehaviour
    {
        ...
        public GameObject rotationSnap;
        public bool inHand;
        ...
        protected virtual void OnAttachedToHand(Hand hand)
        {
            ...
            if (rotationSnap != null)
            {
                ...
                {
                    rotationSnap.transform.localPosition = new Vector3(-0.051f, 0.124f, -0.073f);
                    rotationSnap.transform.localRotation = Quaternion.Euler(new Vector3(0f, 55.07f, -38.1f));
                }
                else
                {
                    rotationSnap.transform.localPosition = new Vector3(0.064f, 0.118f, -0.181f);
                    rotationSnap.transform.localRotation = Quaternion.Euler(new Vector3(0f, 124.6f, -38.1f));
                }
            }
            if(rb != null)
            {
        ...
        protected virtual void OnDetachedFromHand(Hand hand)
        {
            ...
            if (itemcomp != null && GameObject.Find("VRPLAYER") != null)
            {
                Transform player = GameObject.Find("VRPLAYER").transform;
                Vector3 checkloc = player.position;
                checkloc.y += 0.5f;
                if (Vector3.Distance(checkloc, transform.position) <= 0.2f) {
                    Inventory i = hand.player.inventory;
                    i.addItem(itemcomp);
                    itemcomp.destory();
                }
            }
